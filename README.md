...
$ vdj login -r -s
```
If you prefer to not cache your access token, it must be passed to the command via the `-z` flag.
...
lag to `-j`. The exception to this rule is file uploads and imports, which can only be uploaded as projectFiles. For example, a project's files, projectFiles, and projectJobFiles, respectively can be listed by:
```
...
Some commands that apply to projectFiles and projectJobFiles include:
```
$ vdj files download
$ vdj files rm
$ vdj files ls
